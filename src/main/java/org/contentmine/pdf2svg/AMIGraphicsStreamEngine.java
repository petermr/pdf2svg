...
//	private AMIFont amiFont;
     ...
     * @param page PDF Page
     */
    protected AMIGraphicsStreamEngine(PDF2SVGTransformer pdf2svgTransformer, PDPage page) {
        super(page);
...
//    }
    ...
    protected void showGlyph(Matrix textRenderingMatrix, PDFont font, int code, String unicodeChar,
    	...
    	StyleAttributeFactory.convertOldStyleAttributesToCSSAndDelete(svgText); // normalizes to style attribute
    ...
    public void appendRectangle(Point2D pp0, Point2D pp1, Point2D pp2, Point2D pp3) {
        ...
        PDFPage2SVGConverter converter = pdf2svgTransformer.getPdfPage2SVGConverter();
        if (converter == null) {
        	LOG.error("Null converter");
        	return;
        }
        converter.appendChild(path);
//        pdf2svgTransformer.getPdfPage2SVGConverter().appendChild(path);;
    }

    @Override
    public void strokePath() throws IOException {
		LOG.info("STROKEPATH "+linePath);
		if (graphics != null) {
    ...
    public void fillAndStrokePath(int windingRule) throws IOException {
		LOG.info("FILLANDSTROKEPATH "+windingRule);
...
//    		throw new RuntimeException("IMAGE");
    ...
    protected void processType3Stream(PDType3CharProc charProc, Matrix textRenderingMatrix) throws IOException {
    	...
    	LOG.info("****processType3Stream "+charProc+"; "+textRenderingMatrix);
    }
    ...
    public PDAppearanceStream getAppearance(PDAnnotation annotation) {
    	LOG.info("****getAppearance "+annotation);
...
//	private Stroke getStroke() {return null;};
	...
	private String getCurrentPathString() {
    	LOG.info("GET CURRENT PATH STRING " + this.currentPathString);
